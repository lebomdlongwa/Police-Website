import React from "react";
import { IconBaseProps } from "react-icons";
import { Color } from "../colorCodes";
import { CSSProperties } from "styled-components";

export const CrimeSceneIcon = (props: IconProps) => {
  const { h = 15, c = Color.iconGray, w = 15 } = props;

  return (
    <svg
      width={`${w}px`}
      height={`${h}px`}
      version="1.1"
      id="Layer_1"
      viewBox="0 0 511.978 511.978"
    >
      <path
        style={{ color: c } as CSSProperties}
        d="M1.436,145.953L33.42,90.549c1.421-2.453,3.75-4.234,6.484-4.969
	c2.719-0.734,5.641-0.344,8.078,1.062l458.669,264.81c5.094,2.953,6.844,9.469,3.891,14.578l-31.983,55.403
	c-1.422,2.453-3.75,4.234-6.469,4.969c-0.922,0.25-1.844,0.359-2.766,0.359c-1.859,0-3.702-0.484-5.327-1.422L5.327,160.53
	C0.233,157.577-1.517,151.062,1.436,145.953z"
      />
      <g>
        <polygon
          style={{ color: c } as CSSProperties}
          points="351.205,261.698 425.075,304.36 456.309,420.902 382.421,378.248 	"
        />
        <polygon
          style={{ color: c } as CSSProperties}
          points="308.55,335.593 234.661,292.938 203.444,176.389 277.316,219.043 	"
        />
        <polygon
          style={{ color: c } as CSSProperties}
          points="160.789,250.277 86.902,207.622 55.684,91.08 129.556,133.734 	"
        />
      </g>
      <path
        style={{ color: c } as CSSProperties}
        d="M510.542,145.953l-31.984-55.404c-1.422-2.453-3.75-4.234-6.469-4.969
	c-2.734-0.734-5.655-0.344-8.093,1.062L5.327,351.452c-5.094,2.953-6.844,9.469-3.891,14.578l31.983,55.403
	c1.421,2.453,3.75,4.234,6.484,4.969c0.906,0.25,1.828,0.359,2.75,0.359c1.859,0,3.703-0.484,5.328-1.422l458.669-264.81
	C511.745,157.577,513.495,151.062,510.542,145.953z"
      />
      <g>
        <polygon
          style={{ color: c } as CSSProperties}
          points="160.789,261.698 86.902,304.36 55.684,420.902 129.556,378.248 	"
        />
        <polygon
          style={{ color: c } as CSSProperties}
          points="203.444,335.593 277.316,292.938 308.55,176.389 234.661,219.043 	"
        />
        <polygon
          style={{ color: c } as CSSProperties}
          points="351.205,250.277 425.075,207.622 456.294,91.08 382.421,133.734 	"
        />
      </g>
      <path
        style={{ color: c } as CSSProperties}
        d="M255.989,149.359c-58.795,0-106.637,47.826-106.637,106.636c0,58.802,47.842,106.644,106.637,106.644
	c58.794,0,106.637-47.842,106.637-106.644C362.626,197.185,314.783,149.359,255.989,149.359z"
      />
      <g>
        <path
          style={{ color: c } as CSSProperties}
          d="M255.989,277.33c-5.891,0-10.656-4.781-10.656-10.679V213.34c0-5.89,4.766-10.671,10.656-10.671
		s10.671,4.781,10.671,10.671v53.311C266.66,272.549,261.879,277.33,255.989,277.33z"
        />
        <path
          style={{ color: c } as CSSProperties}
          d="M266.66,298.657c0,5.875-4.781,10.656-10.671,10.656c-5.891,0-10.656-4.781-10.656-10.656
		c0-5.89,4.766-10.671,10.656-10.671C261.879,287.986,266.66,292.767,266.66,298.657z"
        />
      </g>
    </svg>
  );
};
